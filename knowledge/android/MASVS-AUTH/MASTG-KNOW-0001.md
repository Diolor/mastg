...
There are two `authenticate` methods available in the `BiometricPrompt` class. One of them expects a [`CryptoObject`](https://developer.android.com/reference/kotlin/android/hardware/biometrics/BiometricPrompt.CryptoObject "CryptoObject"), which adds an additional layer of security for the biometric authentication.
