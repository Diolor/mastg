...
Although [the documentation](https://developer.android.com/reference/kotlin/java/security/SecureRandom#setseed) says the provided seed normally supplements the existing seed, this behavior may differ if an [old security provider](https://android-developers.googleblog.com/2016/06/security-crypto-provider-deprecated-in.html) is used. To avoid these pitfalls, ensure your app targets a modern Android version with an updated provider or explicitly configures a secure provider such as AndroidOpenSSL (or Conscrypt in newer releases).
